<?php
  ...
  if(!empty($_POST["dUn_check"]) && !empty($_POST["dPw_check"])){
    ...
    foreach ($results as $row){
      ...
      if(($row['name']==$_POST["dUn_check"]) && ($row['pw']==$_POST["dPw_check"])){
        if(!empty($used_newUn) && !empty($used_newPw)){
          ...
          $sql = 'UPDATE welcometo SET name=:name,pw=:pw WHERE id=:id';
    ...
    $errormessage_ec="<br>仮ユーザ名または仮パスワードが間違っているか、新しいユーザ名、パスワードが入力されていません。";
    echo $errormessage_ec;
  }
  else{

    $errormessage_ec="<br>仮ユーザ名または仮パスワードが入力されていません。";
  }
?>
<!DOCTYPE html>
<html lang="en">
...
<body>
<!-- <p>入力されたメールアドレスに記載された仮ユーザ名と仮パスワードを入力してください。<?php echo $errormessage_ec;?></p> -->
